...
- 画像フィルター機能
  - 主にGarden案件で画像編集機能があるが、画像のフィルター機能は存在していないので導入できると思い作ってみたかった
- 「WASM = 速い」のは知っているが、使い所を知りたい・考えたい
...
## 課題 4: JS / WASM / WebGL ベンチマーク

![ベンチマーク結果](images/image.png)

- 小さい画像（65,536px）ではWASMが最速、WebGLはJSとほぼ同等
- WebGLはGPUへのデータ転送コストがあり、**画素数が少ないと逆にオーバーヘッドが目立つ**
- 画素数が大きくなるほどGPUの並列処理が効いてWebGLが有利になる
...
## まとめ: Web計算アーキテクチャの使い分け

WASMもWebGLも万能ではない — **処理特性に応じて最適な計算レイヤーを選ぶ**のが重要

| 処理特性              | 最適な技術   | 例                               |
| --------------------- | ------------ | -------------------------------- |
| 汎用ロジック・DOM操作 | JS (V8 JIT)  | UI、文字列処理、API呼び出し      |
| CPU数値演算の高速化   | WASM         | 画像処理、暗号化、圧縮           |
| 大量データの並列演算  | WebGL/WebGPU | フィルター、物理シミュレーション |

- 今回の実証: 小画像ではWASMが最速、大画像ではWebGLが最速
- **「何で書くか」ではなく「どこで計算するか」** がアーキテクチャ設計の鍵
- JS / WASM(CPU) / WebGL・WebGPU(GPU)を組み合わせて適材適所で使う時代へ

---

## 残課題

- **asconfig.jsonの設定値による速度比較**
  - `optimizeLevel`（0〜3）: 最適化の強さ
  - `shrinkLevel`（0〜2）: サイズ縮小 vs 速度のトレードオフ
  - `noAssert`（true/false）: 境界チェック除去による高速化
  - 現在は`optimizeLevel: 3`, `shrinkLevel: 0`で固定しているが、
    各設定の組み合わせで実測比較し、最適な設定を検証したい
- **Rust/C++でのWASM実装との比較**
  - 同じdiffアルゴリズムをRustで書いた場合の速度差を検証したい
  - ただ、Rustの学習まで行けなかったので、これは今後の課題
