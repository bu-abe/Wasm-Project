...
グレースケール、1,507,024px での比較:

|          |     時間 | vs JS     |
| -------- | -------: | --------- |
| JS       | 115.0 ms | —         |
| WASM     |  36.3 ms | 3.2x 速い |
| **WebGL** | **13.6 ms** | **8.5x 速い** |

- WASMはJSの3.2倍速いが、WebGLはさらにWASMの2.7倍速い
- CPUは150万ピクセルを1つずつ処理、GPUは数千コアで**一斉に処理**
- 一律フィルターのような処理はGPUの独壇場
...
- **Rust/C++でのWASM実装との比較**
  - diff-toolが遅いのはAssemblyScriptの自前ランタイム（GC、配列管理）が原因
  - Rustなら所有権システムでGC不要、配列操作も低レベルに最適化される
